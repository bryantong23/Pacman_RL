        ...
        for i in range(self.iterations):
            ...
            for j in range(len(states)):
                ...
                for k in range(len(actions)):
                    qVal = self.computeQValueFromValues(states[j], actions[k])
                    if qVal > maxVal:
                        maxVal = qVal
                        tempVals[states[j]] = maxVal
        ...
        for i in range(self.iterations):
            ...
            if not self.mdp.isTerminal(currentState):
                ...
                for j in range(len(actions)):
                    tempVal = 0
                    for nextState, probability in self.mdp.getTransitionStatesAndProbs(currentState, actions[j]):
                        tempVal += (probability * (self.mdp.getReward(currentState, actions[j], nextState) + self.discount * values[nextState]))
                    if tempVal > maxVal:
                        maxVal = tempVal
                self.values[currentState] = maxVal
